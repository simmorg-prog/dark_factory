...
**Changes:** v1.1 — Added E5-00. v1.2 — Added E5-08. v1.3 — Added Practice Premium Principle. v1.4 — Split E5-01, moved to Wave 2. v1.5 — Added E1-07, E4-00, Atomic Specification Principle, five authoring notes, four glossary terms. v1.6 — Added E1-08 (LANGUAGE.md). v1.7 — Added RFC 2119 Precision Principle as a named core principle; expanded E1-08, E1-07, E1-06 content briefs; updated quality checks.
...
| E1-08 | [LANGUAGE.md — Structural Vocabulary](./00-foundations/LANGUAGE.md) | 🔜 Pending |
...
> **Total items:** 46 | **Done:** 2 | **In Progress:** 0 | **To Do:** 44
```
...
| E1-08 | LANGUAGE.md — Structural Vocabulary | Foundations | 1 | P2 | S | 🔜 To Do |
...
| Wave | Focus | Items |
|---|---|---|
| 1 | Foundations | E1-01 ✅ E1-02 ✅ E1-05 E1-03 E1-04 E1-06 E1-07 E1-08 |
...
3. **Principle 1 — Rigour from the outset, not rigour all at once** — formatted as a named blockquote. The compounding argument. The spectrum diagram (linked SVG). RFC 2119 as the evidence that precision and brevity coexist — a MUST in a user story acceptance criterion is as precise as a MUST in a 500-page document, and costs almost nothing to write. Include a callout box: *"The minimum viable precision standard at Stage 1 is three things: include the RFC 8174 boilerplate, bind every obligation keyword to a named actor, and accompany every SHOULD with a rationale clause. This costs minutes. Not having it at Stage 4 costs months."*
...
6. **Principle 4 — Normative language must be declared and calibrated** — formatted as a named blockquote. This principle is new and cross-references LANGUAGE.md. A specification that uses MUST without the RFC 8174 boilerplate is expressing aspiration, not obligation. A SHOULD without a rationale clause is a weak MUST with the information removed. A keyword applied to an unnamed subject has no conformance meaning. The RFC 2119 Precision Principle (defined in full in `LANGUAGE.md`) is the minimum standard for any normative claim in this framework, at any maturity stage.
7. The practical implication — one tight closing section. What to do tomorrow, regardless of maturity stage: use RFC 2119 semantics on the next requirement you write. Include the boilerplate. Bind every keyword to a named actor. Add a rationale to every SHOULD. Make acceptance criteria explicit and binary. Name what is out of scope. Version your specifications. The corpus grows from there.
8. Navigation links — include explicit link to `./LANGUAGE.md` for the RFC 2119 Precision Principle full text
...
- Definition: Obligation keywords (MUST, SHOULD, MAY, MUST NOT, SHOULD NOT) carry normative force only when the six rules of the RFC 2119 Precision Principle are met — as defined in `LANGUAGE.md` Section 3
- Failure: Missing boilerplate (Rule 1 violation); keywords applied to unnamed subjects (Rule 2 violation); SHOULD used as a hedge without rationale (Rules 3 and 4 violation); lowercase instances treated as normative (Rule 5 violation)
- Ideal: All six rules met — boilerplate present verbatim, every keyword bound to a named conformance role, MUST/SHOULD/MAY calibrated correctly, every SHOULD carries a rationale clause, lowercase non-normative
- Test: Can you remove the boilerplate and have the document still read correctly? If yes, the keywords are decorative, not normative. Does every SHOULD have a rationale clause? If no, the document has Rule 4 violations.
- Reference: See [LANGUAGE.md — Normative Language section](./LANGUAGE.md) for the full six rules and keyword decision rubric
...
### 7.11 — E1-08: LANGUAGE.md — Structural Vocabulary

**File:** `00-foundations/LANGUAGE.md`  
**No diagrams required.**

**Purpose:** Inspired directly by Matt Pocock's skills framework (`mattpocock/skills`), which uses a `LANGUAGE.md` file to provide a shared, precise vocabulary for architectural reasoning — distinct from the project's domain vocabulary. That framework demonstrates that consistent terminology is not just helpful but *load-bearing*: it prevents Claude Code from drifting into synonyms that subtly shift meaning and produce inconsistent outputs across sessions.

The `glossary.md` in this project covers **domain vocabulary** — what intent engineering, harness, specification corpus, and Dark Factory mean. `LANGUAGE.md` covers **structural vocabulary** — the terms used to describe how the dark_factory framework itself is organised, navigated, and extended. Claude Code needs both to operate correctly.

**File header:**
```markdown
> **E1-08 · Foundations · Wave 1**  
> Structural vocabulary for the AI Operational Maturity Framework repository.  
> Defines how the framework is organised, how its components relate, and 
> the precise terms Claude Code must use when navigating, extending, or 
> discussing the framework's own architecture.  
...
1. **Why this file exists** — one short paragraph. The glossary defines what the framework is *about*. This file defines what the framework *is* — its own structural anatomy. Two distinct vocabularies serve two distinct purposes. Using "document" when you mean "backlog item," or "section" when you mean "wave," or "chapter" when you mean "epic" produces inconsistency that accumulates into confusion at scale. Consistent structural language is the whole point.

2. **Framework structure terms** — the precise vocabulary for the framework's own architecture:

**Epic**
A thematic group of related backlog items. The six epics are: Foundations, Stage Deep Dives, Actors, Artefacts, Examples, and Strategy. An epic is not a folder — it is a logical grouping that may span multiple folders.
*Avoid*: chapter, section, theme, category.

**Backlog Item**
A discrete unit of content work with a unique ID, title, epic, wave, priority, effort estimate, and status. The atomic unit of work in the framework. Identified by a compound ID: epic prefix + sequence number (e.g. E1-07, E4-00).
*Avoid*: task, ticket, story, document, page.

**Wave**
A breadth-first execution batch. All Wave 1 items are completed before Wave 2 items begin. A wave groups items by thematic focus, not by difficulty or size.
*Avoid*: sprint, phase, iteration, stage (reserved for the six maturity stages).

**Stage**
One of the six maturity levels of the framework: Prompt Engineering (1) through Environment Engineering (6). Not to be confused with a wave.
*Avoid*: level (ambiguous with CMMI levels), tier, phase.

**Master Brief**
The single authoritative instruction document for Claude Code. Versioned with semantic versioning. The Master Brief is the specification that governs all content production. It subsumes all prior verbal or conversational instructions.
*Avoid*: brief, instructions, spec, guide.

**Authoring Note**
A content direction decision stored in Section 10 of the Master Brief for a backlog item not yet being written. An authoring note is not a full content brief — it records a framing decision, a naming convention, or a structural requirement that must be applied when the item is eventually executed.
*Avoid*: note, comment, reminder, guideline.

**Content Brief**
The full specification for a backlog item within the Master Brief — file path, header, diagrams, content structure, and authoring guidance. Every item being actively executed must have a content brief in the Master Brief before Claude Code begins writing it.
*Avoid*: spec, brief (standalone — always "content brief" to distinguish from Master Brief), instructions.

**Wave Status** 
The execution state of a backlog item: `To Do`, `In Progress`, or `Done`. Maintained in both `BACKLOG.md` and the README wave status table.
*Avoid*: open, closed, complete, pending (use the exact three-state vocabulary).

**Diagram pair**
A `.drawio` source file and its corresponding exported `.svg` — always produced together, always committed together, always stored in `/diagrams/`. A diagram pair is never split: committing one without the other is a quality defect.
*Avoid*: diagram (ambiguous — always specify which file type or say "diagram pair").

**Relative link**
An internal cross-reference using a path relative to the current file (e.g. `./glossary.md`, `../diagrams/maturity-curve.svg`). All internal links in this framework MUST be relative links. Absolute GitHub URLs are a quality defect.
*Avoid*: link, URL, reference (when the type matters — use "relative link" or "absolute URL" explicitly).

3. **Depth and the deletion test** — a short section applying the depth concept from Ousterhout (as articulated in Pocock's skills framework) to the framework's own structure:

A backlog item is **deep** when a significant amount of content and understanding sits behind a small, clear title. A backlog item that requires the reader to read everything it links to in order to understand what it covers is **shallow** — its interface is as complex as its implementation.

The **deletion test** applied to framework structure: imagine removing a backlog item. Does the framework's coverage become meaningfully incomplete, or was the item duplicating something already covered? If content would be lost that no other item covers, the item was earning its keep. If the same material would appear elsewhere, the item is a pass-through and should be merged or removed.

This applies to diagrams too: if removing a diagram from a document leaves the reader able to understand the content from the prose alone, the diagram was decorative. If removing it creates a genuine gap in comprehension, it was load-bearing.

4. **Rejected terms** — explicit list of terms to avoid and what to use instead:

| Avoid | Use instead | Reason |
|---|---|---|
| document, page, chapter | backlog item | "Document" is too broad — every `.md` file is a document |
| task, ticket, story | backlog item | Borrows from project management vocabularies that don't apply |
| phase, iteration, sprint | wave | Borrows from software delivery vocabularies |
| level | stage (maturity) or CMMI level | "Level" is ambiguous between maturity stages and CMMI |
| spec, brief | content brief or Master Brief | Ambiguous — always qualify |
| diagram | diagram pair | Never commit one without the other |
| link, URL | relative link or absolute URL | The distinction is a quality check, not a style preference |
| boundary | seam (architectural) or scope (specification) | "Boundary" is overloaded with DDD meaning |
| service, component, API | module, interface, seam | Borrows from code architecture vocabulary inappropriately |

5. **Navigation links** — back to Glossary, forward to Reader Guide

---

**ADDITIONAL SECTION FOR LANGUAGE.md — Normative Language: The RFC 2119 Precision Principle**

This is the most important section in `LANGUAGE.md`. It defines how obligation keywords are used throughout every normative document in the framework. Insert it as Section 3 of the file, immediately after the Framework Structure Terms section and before the Depth and Deletion Test section. Renumber subsequent sections accordingly.

**Section title:** `## Normative Language — The RFC 2119 Precision Principle`
...
Add this table immediately after the six rules, as a practical reference for authors:
...
### 7.12 — E4-00: Specification Examples — Reference Library
...
11. 📝 Create `00-foundations/LANGUAGE.md` — per Section 7.11 (no diagrams)
12. 📝 Create `05-strategy/vision-ai-native.md` — per Section 7.6 (no diagrams)
13. 📝 Create `05-strategy/wardley-map.md` + 2 diagrams — per Section 7.7
14. 📝 Create `05-strategy/enterprise-knowledge-work-map.md` + 2 diagrams — per Section 7.5
15. 📝 Create `05-strategy/maturity-assessment-framework.md` + 2 diagrams — per Section 7.8
16. 📝 Create `05-strategy/maturity-assessment-tool.md` + 1 diagram — per Section 7.9
17. 📝 Create `02-artefacts/specification-examples.md` — per Section 7.12 (no diagrams)
18. 🔍 Final check — verify all internal links are relative, all diagrams are linked not embedded, framework name is consistent throughout, LANGUAGE.md terms used throughout
...
- [ ] The five named principles appear consistently: **Specification Debt Principle**, **Strategic Misalignment Principle**, **Practice Premium Principle**, **Atomic Specification Principle**, **RFC 2119 Precision Principle**
...
- [ ] LANGUAGE.md structural vocabulary used throughout — no rejected terms (document/chapter/task/phase/level/diagram/link in isolation)

**RFC 2119 Precision Principle — six rule checks (apply to every normative document):**
- [ ] Rule 1: Document contains the verbatim RFC 8174 boilerplate declaration in a Conventions section before any normative content
- [ ] Rule 2: Every MUST/SHOULD/MAY/MUST NOT/SHOULD NOT is applied to a named conformance role, not an abstract system or process
- [ ] Rule 3: Keyword strength is correctly calibrated — no SHOULD used as a hedge for an uncertain MUST
- [ ] Rule 4: Every SHOULD and SHOULD NOT is accompanied by a rationale clause stating either the exceptional circumstance or the harm being prevented
- [ ] Rule 5: No lowercase must/should/may instances are treated as normative obligations
- [ ] Rule 6: The six rules are applied to all normative documents; purely descriptive documents (glossaries, reader guides) are correctly excluded
